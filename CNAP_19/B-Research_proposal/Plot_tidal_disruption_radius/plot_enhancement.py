...
for k in range(N):
	q[k]=MNS/MBH[k]
	Egg[k]=(0.49*q[k]**(2./3.))/(0.6*q[k]**(2./3.)+np.log(1+q[k]**(1./3.)))
	# Egg[k]=(3/q[k])**(1./3.)
	# ( RNS / Eggleton(MNS/MBH) ) / ISCO radius
	# - - - RNS_inf
	ratio[0][k]=(RNS_inf/(6.*G*MBH[k]*Msun/c**2.))/Egg[k]
	# - - - RNS_sup
	ratio[1][k]=(RNS_sup/(6.*G*MBH[k]*Msun/c**2.))/Egg[k]
...
fig1.plot(MBH,(1./3.)*np.ones(N),linestyle='dotted',linewidth=2,color='k')
fig1.plot(MBH,np.ones(N),linestyle='dashed',linewidth=2,color='k')
# fig1.plot(MBH[::50],ratio[0][::50],linestyle='solid',linewidth=3,color='g',alpha=0.3)
# fig1.plot(MBH,ratio[0][:],linestyle='dashed',linewidth=3,color='r')
# fig1.plot(MBH[0:13],ratio[1][0:13],linestyle='solid',linewidth=3,color='g')
# fig1.plot(MBH[13::],ratio[1][13::],linestyle='solid',linewidth=3,color='g',alpha=0.3)
# fig1.grid(which='minor', linestyle='dotted', linewidth=2, alpha=0.9)
fig1.set_ylim(0.,1.8)
fig1.set_xlim(3.,30.)
plt.text(3.2,(1./3.)+0.02, 'Event horizon', rotation=0, fontsize=fontsize)
plt.text(8.8,1.+0.02, 'Innermost stable circular orbit (ISCO)', rotation=0, fontsize=fontsize)
fig1.fill_between(MBH,0.,1./3.,color='grey')
fig1.fill_between(MBH[0:13],ratio[1][0:13],1.,color='g')
y4 = np.minimum(np.ones(N),ratio[1][:])
y5 = np.maximum((1./3.)*np.ones(N),ratio[0][:])
fig1.fill_between(MBH,y4,y5,color='g',alpha=0.3)
...
fig1.set_xlabel(r'Mass of the black hole (in solar masses)',fontweight='bold',fontsize=fontsize)
fig1.set_ylabel(r'Tidal disruption radius / ISCO radius', fontweight='bold', fontsize=fontsize)
...
# fig.tight_layout()
fig.savefig(outputs+'tidal_disruption_radius.png',bbox_inches='tight')
# plt.show()
# stop
# fig1.set_ylim(1.,5.)
# fig1.set_xlim(1.,2.1)
